...
TEST_CASE("Dice::getTupleCount", "[Dice]") {
	SECTION("Heads or Tails") {
		...
		TupleCount count = dice.getTupleCount({Face::HEADS(), Face::HEADS()});
		REQUIRE(count.size() == 3);
		INFO("0: " << count[0] << " 1: " << count[1] << " 2: " << count[2]);
		CHECK(count[0] == 6);
		CHECK(count[1] == 20);
		CHECK(count[2] == 6);
		std::size_t sum =
	...
	SECTION("6D6, {1,2}") {
		TupleCount count = Dice(6, Die::D6()).getTupleCount({Face::ONE(), Face::TWO()});
		REQUIRE(count.size() == 4);
		INFO("0: " << count[0] << " 1: " << count[1] << " 2: " << count[2] << " 3:"
							 << count[3]);
		CHECK(count[0] == 27154);
		CHECK(count[1] == 17532);
		CHECK(count[2] == 1950);
...
TEST_CASE("Dice::getTupleStat", "[Dice]") {
		SECTION("time for 6D6, {1, 2}") {
			...
			TupleStat stat = die.getTupleStat({Face::ONE, Face::TWO});
	...
	SECTION("Heads or Tails") {
		Dice dice(5, {Face::HEADS(), Face::TAILS()});
		TupleStat stat = dice.getTupleStat({Face::HEADS(), Face::HEADS()});
		REQUIRE(stat.size() == 3);
	...
	SECTION("5D{H, H, T}, {H, H}") {
		...
		TupleStat stat = die.getTupleStat({Face::HEADS(), Face::HEADS()});
		REQUIRE(stat.size() == 3);
		INFO("0: " << stat[0] << " 1: " << stat[1] << " 2: " << stat[2]);
	...
	SECTION("6D6, {1,2}") {
		TupleStat stat = Dice(6, Die::D6()).getTupleStat({Face::ONE(), Face::TWO()});
		REQUIRE(stat.size() == 4);
		INFO(
